...
@Yonebot.on_callback_query(filters.regex(pattern=r'^(playlist)$'))
async def p_cb(b, cb):
    global que    
...
@Yonebot.on_callback_query(filters.regex(pattern=r'^(play|pause|skip|leave|puse|resume|menu|cls)$'))
@cb_admin_check
async def m_cb(b, cb):
    global que    
    qeue = que.get(cb.message.chat.id)
...
async def play(_, message: Message):
    ...
    wew = usar.id
    try:
        #chatdetails = await Yonemusic.get_chat(chid)
...
async def deezer(client: Client, message_: Message):
        ...
        lmoa = await client.get_chat_member(chid,wew)
           ...
           for administrator in administrators:
                      if administrator == message_.from_user.id:  
                          ...
                          try:
                              await Yonemusic.join_chat(invitelink)
                              await Yonemusic.send_message(message_.chat.id,"I joined this group for playing music in VC")
                              await lel.edit(
                                  "<b>helper userbot joined your chat</b>",
                              )
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"𝐘𝐨𝐧𝐞 𝐌𝐮𝐬𝐢𝐜 𝐁𝐨𝐭🎶🎸= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("𝐘𝐨𝐧𝐞 𝐌𝐮𝐬𝐢𝐜 𝐁𝐨𝐭🎶🎸=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"𝐘𝐨𝐧𝐞 𝐌𝐮𝐬𝐢𝐜 𝐁𝐨𝐭🎶🎸=#️⃣ Queued at position {position}",
